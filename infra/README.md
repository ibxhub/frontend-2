...
This module provisions the AWS resources required by `frontend-2`.
...
- **Secrets Manager entry** at `frontend-2/<env>/config` holding `jwt_signing_key`, `okta_issuer`, `okta_client_id`, `okta_client_secret`, `okta_redirect_url`. Initial values are `REPLACE_ME` placeholders — the **Platform Admin** must fill them after the first apply.
...
| ------------- | ---------------------- | ------------------------------------------ |
| `app_name`    | `frontend-2`           | Prefix for resource names                  |
...
aws secretsmanager put-secret-value \
  --secret-id frontend-2/dev/config \
