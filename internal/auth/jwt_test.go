...
func TestMintAndVerify_RoundTrip(t *testing.T) {
	tok, err := Mint(testKey, "frontend-2", "alice@infoblox.com", "Alice")
	if err != nil {
		t.Fatalf("Mint: %v", err)
	}
	...
	if claims.Issuer != "frontend-2" {
		t.Errorf("Issuer = %q, want frontend-2", claims.Issuer)
	}
}
...
func TestMint_TokenShape(t *testing.T) {
	tok, err := Mint(testKey, "frontend-2", "a@b.c", "A B")
	if err != nil {
		t.Fatalf("Mint: %v", err)
	}
